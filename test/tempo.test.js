...
describe('tempo CLI', () => {
  ...
  test('reset command exists but not implemented', async () => {
    const env = { TEMPO_STORE: testStorePath };
    
    const resetResult = await runTempo(['reset'], env);
    // Currently returns exit code 2 (not implemented)
    assert.strictEqual(resetResult.code, 2);
    assert.match(resetResult.stderr, /not implemented/);
  });
  
  test('list command exists but not implemented', async () => {
    const env = { TEMPO_STORE: testStorePath };
    
    const listResult = await runTempo(['list'], env);
    // Currently returns exit code 2 (not implemented)  
    assert.strictEqual(listResult.code, 2);
    assert.match(listResult.stderr, /not implemented/);
  });
